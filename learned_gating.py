...
class ImportanceGatingHead(nn.Module):
    ...
    def __init__(self, d_model: int):
        ...
        # Initialize bias to -5.0 so initial weights default close to 1.0
...
class AutonomousWeightedTransformerDecoder(nn.Module):
    ...
    def forward(
        ...
        """
        idx: [B, N] token indices
        custom_weights: Optional[B, N] external weights (if provided)
        returns: (logits [B, N, vocab_size], layer_attn_weights, token_weights [B, N])
        """
        B, N = idx.shape
        device = idx.device
        ...
        # Pass through Transformer layers using logit_bias entrypoint
        x = h
        layer_attn_weights = []
