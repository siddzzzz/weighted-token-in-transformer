...
class WeightedMultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        if token_weights is not None and entrypoint != "baseline":
            # Ensure token_weights are non-negative
            w = torch.clamp(token_weights, min=1e-4) # [B, N]
            
            if entrypoint in ["k_scale"]:
                # Scale key vectors K_j by sqrt(w_j)
                w_k = torch.sqrt(w).view(B, 1, N, 1) # [B, 1, N, 1]
                K = K * w_k

            if entrypoint in ["v_scale", "combo"]:
                # Scale value vectors V_j by w_j
...
class WeightedTransformerDecoder(nn.Module):
    ...
    def forward(
        ...
        token_weights: [B, N] token weights
        returns: (logits [B, N, vocab_size], list of attn_weights per layer)
        """
        B, N = idx.shape
        device = idx.device
